...
class Utils:
    ...
    def get_market_value_safe(player_data: pd.Series) -> str:
        ...
        possible_columns = [
            ...
            'Valeur_marche', 'valeur_de_marche', 'MarketVal', 'MV', 'mv',
            'Transfer_Value', 'TransferValue', 'Worth', 'worth', 'Cost', 'cost'
        ]
        
        # Debug: afficher les colonnes disponibles (optionnel)
        # print(f"Colonnes disponibles pour {player_data.get('Joueur', 'Unknown')}: {list(player_data.index)}")
        ...
        for col in player_data.index:
            if pd.notna(player_data.get(col)):
                try:
                    ...
                    # Valeurs typiques de valeurs marchandes (entre 100K et 500M)
        ...
        # Essayer toutes les colonnes une par une pour trouver des valeurs numériques raisonnables
        for col in player_data.index:
            if pd.notna(player_data.get(col)):
                try:
                    value = float(player_data[col])
                    # Valeurs qui pourraient être des valeurs marchandes en euros
                    if 50_000 <= value <= 300_000_000:
                        # Vérifier si c'est probablement une valeur marchande
                        if col.lower() not in ['age', 'âge', 'minutes', 'matchs', 'buts', 'passes', 'tirs', 'position']:
                            formatted_value = Utils.format_market_value(value)
                            if formatted_value != "N/A":
                                return formatted_value
                except (ValueError, TypeError):
...
class UIComponents:
    ...
    def render_similar_player_card(player_info: Dict, rank: int):
        ...
        # Debug: afficher les colonnes disponibles pour ce joueur (optionnel)
        # st.write(f"Debug {player_info['joueur']}: {list(player_data.index)}")
...
class TabManager:
    ...
    def render_similar_players_tab(selected_player: str, df: pd.DataFrame):
        ...
        st.markdown("<h3 class='subsection-title-enhanced'>📊 Métriques Analysées pour le Calcul de Similarité</h3>", unsafe_allow_html=True)
        ...
        # Informations détaillées sur les valeurs marchandes
        st.markdown("---")
        st.markdown("<h3 class='subsection-title-enhanced'>💰 Analyse des Valeurs Marchandes</h3>", unsafe_allow_html=True)
        
        # Récupérer les valeurs marchandes de tous les joueurs
        target_data = df[df['Joueur'] == selected_player]
        if not target_data.empty:
            ...
            # Afficher les valeurs marchandes
...
if __name__ == "__main__":
    main(), '').replace(',', '').replace(' ', '').strip()
                ...
                # Gérer les cas comme "50.5M" ou "2.3K" ou "1.2B"
                multiplier = 1
                ...
                # Enlever les autres caractères non numériques
        ...
        try:
            ...
            if value <= 0:
                return "N/A"
            if value > 1_000_000_000_000:  # Plus de 1000 milliards semble irréaliste
                return "N/A"
        except (ValueError, TypeError):
            return "N/A"
